  ...
  const handleDecision = (decision: Decision) => {
    ...
    if (currentIndex < articles.length - 1) {
      setTimeout(() => {
        ...
        setIsListOpen(false); // Chiude su mobile se aperto
      }, 150);
    }
  };
  ...
  return (
    <div className="h-screen flex flex-col bg-slate-50 overflow-hidden relative">
      ...
      {(isListOpen || isKeywordsOpen) && (
        <div 
          className="fixed inset-0 bg-slate-900/40 backdrop-blur-[2px] z-[60] lg:hidden animate-in fade-in"
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-[32px] shadow-2xl max-w-sm w-full p-8 animate-in zoom-in duration-300">
            ...
            <div className="space-y-3">
              ...
              <button onClick={() => setShowExitModal(false)} className="w-full py-3 text-slate-400 font-bold hover:text-slate-600 transition-colors text-xs uppercase tracking-widest">
                Cancel
              </button>
            </div>
          </div>
      ...
      <header className="h-16 lg:h-18 bg-white border-b px-4 lg:px-6 flex items-center justify-between shadow-sm shrink-0 z-50">
        <div className="flex items-center gap-2 lg:gap-4">
          ...
          <div className="flex items-center gap-2">
             <div className="hidden sm:flex h-8 w-8 lg:h-9 lg:w-9 bg-indigo-700 rounded-xl items-center justify-center text-white shadow-lg shadow-indigo-100">
                <Microscope size={18} />
             </div>
             <div className="flex flex-col">
                <span className="font-black text-slate-900 text-sm lg:text-lg tracking-tighter leading-none uppercase">SCREENY <span className="text-indigo-600">PRO</span></span>
                <span className="hidden lg:block text-[7px] font-black text-slate-400 uppercase tracking-[0.25em]">BY SRSG</span>
             </div>
          </div>
        </div>

        <div className="flex-1 max-w-xs lg:max-w-xl px-4 lg:px-12">
           <div className="flex flex-col items-center">
              <span className="text-[8px] lg:text-[10px] font-black text-slate-400 uppercase tracking-[0.15em] lg:tracking-[0.2em] mb-1.5">{remainingCount} studies left</span>
              <div className="w-full h-1.5 lg:h-2.5 bg-slate-100 rounded-full overflow-hidden shadow-inner border border-slate-200/50">
                <div 
        ...
        <div className="flex items-center gap-1 lg:gap-2">
          ...
          <button onClick={handleSaveAll} className="hidden lg:flex bg-indigo-600 text-white px-5 py-2.5 rounded-xl text-xs font-black items-center gap-2 hover:bg-indigo-700 transition-all shadow-md active:scale-95">
            <Save size={14}/> Save
          </button>
          <button onClick={() => setShowExitModal(true)} className="p-2 text-slate-400 hover:text-rose-500 transition-colors">
            <X size={24}/>
          </button>
        </div>
      </header>
      ...
      <div className="flex-1 flex overflow-hidden relative">
        {/* ASIDE LEFT: Article List (Drawer on mobile) */}
        <aside className={`
          fixed lg:static inset-y-0 left-0 z-[70] lg:z-10
          w-72 lg:w-72 bg-white border-r flex flex-col shrink-0
          transition-transform duration-300 ease-in-out
          ${isListOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}
        `}>
          <div className="p-4 flex items-center justify-between lg:hidden border-b bg-indigo-50/30">
            <span className="font-black text-xs text-indigo-900 uppercase tracking-widest">Studies List</span>
            <button onClick={() => setIsListOpen(false)} className="p-1 text-indigo-900"><ChevronLeft size={20}/></button>
          </div>
          <div className="p-4 space-y-3 border-b">
            <div className="relative">
              ...
              <input 
                ...
                placeholder="Search..." 
                className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-100 rounded-xl text-[11px] font-bold outline-none" 
              />
            </div>
        ...
        <main className="flex-1 flex flex-col bg-white overflow-hidden border-r relative">
          {currentArticle ? (
            <>
              <div className="flex-1 overflow-y-auto custom-scrollbar p-6 lg:p-14">
                <div className="max-w-2xl mx-auto space-y-6 lg:space-y-8">
                  <div className="flex flex-wrap gap-2 pb-6 border-b border-slate-100">
                     {settings.assignedEmails.map(email => {
                        ...
                        return (
                           <div key={email} className={`flex items-center gap-2 px-2.5 py-1.5 rounded-lg border text-[10px] ${badge.class} shadow-sm`}>
                              <UserCircle size={12} className="opacity-40" />
                              <span className="font-black uppercase tracking-tight">{email === myEmail ? 'You' : email.split('@')[0]}</span>
                           </div>
                        );
                     })}
                  </div>
                  ...
                  <div className="space-y-4 lg:space-y-5">
                    <div className="flex flex-wrap items-center gap-2">
                      <div className="flex items-center gap-1.5 bg-indigo-50 text-indigo-600 px-2.5 py-1 rounded-lg border border-indigo-100">
                        <BookOpen size={10}/>
                        <span className="text-[8px] lg:text-[9px] font-black uppercase tracking-widest truncate max-w-[150px] lg:max-w-[200px]">{currentArticle.journal}</span>
                      </div>
                      <span className="bg-slate-100 text-slate-500 px-2.5 py-1 rounded-lg text-[8px] lg:text-[9px] font-black uppercase tracking-widest border border-slate-200">PMID: {currentArticle.pmid}</span>
                    </div>
                    <h1 className="text-lg lg:text-2xl font-black text-slate-900 leading-tight tracking-tight">{currentArticle.title}</h1>
                    <div className="p-3 bg-slate-50 rounded-xl border border-dashed border-slate-200 max-h-32 lg:max-h-48 overflow-y-auto">
                       <span className="text-[9px] lg:text-[10px] font-bold text-slate-500 italic leading-snug">{currentArticle.authors}</span>
                    </div>
                    <div 
                      className="pt-4 lg:pt-6 text-[13px] lg:text-[15px] text-slate-700 leading-relaxed font-serif-reading selection:bg-indigo-100"
              ...
              {/* BOTTOM ACTIONS: Sticky on all devices */}
              <div className="h-20 bg-white/80 backdrop-blur-md border-t flex items-center justify-center gap-2 lg:gap-3 px-4 lg:px-8 shrink-0 shadow-[0_-4px_20px_rgba(0,0,0,0.05)] z-20">
                 <button onClick={() => handleDecision('exclude')} className="flex-1 max-w-[120px] lg:max-w-[150px] py-3 lg:py-4 bg-white border-2 border-rose-100 text-rose-500 rounded-xl font-black hover:bg-rose-600 hover:text-white hover:border-rose-600 transition-all flex items-center justify-center gap-1.5 lg:gap-2 text-[10px] lg:text-[11px] group active:scale-95">
                    <X size={14} className="group-hover:scale-110 transition-transform"/> EXCLUDE
                 </button>
                 <button onClick={() => handleDecision('maybe')} className="flex-1 max-w-[120px] lg:max-w-[150px] py-3 lg:py-4 bg-white border-2 border-amber-100 text-amber-500 rounded-xl font-black hover:bg-amber-600 hover:text-white hover:border-amber-600 transition-all flex items-center justify-center gap-1.5 lg:gap-2 text-[10px] lg:text-[11px] group active:scale-95">
                    <HelpCircle size={14} className="group-hover:scale-110 transition-transform"/> MAYBE
                 </button>
                 <button onClick={() => handleDecision('include')} className="flex-1 max-w-[120px] lg:max-w-[150px] py-3 lg:py-4 bg-white border-2 border-emerald-100 text-emerald-500 rounded-xl font-black hover:bg-emerald-600 hover:text-white hover:border-emerald-600 transition-all flex items-center justify-center gap-1.5 lg:gap-2 text-[10px] lg:text-[11px] group active:scale-95">
                    <Check size={14} className="group-hover:scale-110 transition-transform"/> INCLUDE
                 </button>
              </div>
            </>
          ) : (
            <div className="flex-1 flex flex-col items-center justify-center text-slate-300">
               ...
               <p className="font-bold">Select a study.</p>
            </div>
          )}
        </main>

        {/* ASIDE RIGHT: Keywords (Drawer on mobile) */}
        <aside className={`
          fixed lg:static inset-y-0 right-0 z-[70] lg:z-10
          w-80 lg:w-80 bg-slate-50 flex flex-col shrink-0 border-l
          transition-transform duration-300 ease-in-out
          ${isKeywordsOpen ? 'translate-x-0' : 'translate-x-full lg:translate-x-0'}
        `}>
          <div className="p-4 flex items-center justify-between lg:hidden border-b bg-indigo-50/30">
            <button onClick={() => setIsKeywordsOpen(false)} className="p-1 text-indigo-900 flex items-center gap-1 font-bold text-xs"><ChevronLeft size={16}/> BACK</button>
            <span className="font-black text-xs text-indigo-900 uppercase tracking-widest">Keywords Manager</span>
          </div>
            ...
            <form onSubmit={handleAddKeyword} className="space-y-3">
              ...
              <div className="relative">
                <input 
                  ...
                  className="w-full pl-4 pr-10 py-2.5 bg-white border border-slate-200 rounded-xl text-[11px] font-bold outline-none focus:ring-2 ring-indigo-500/10"
                />
                <button type="submit" className="absolute right-2 top-1.5 p-1.5 bg-indigo-600 text-white rounded-lg">
                  <Plus size={14}/>
                </button>
              </div>
            </form>
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-6 lg:space-y-8 custom-scrollbar">
            <section>
              ...
              <div className="space-y-2">
                {includeKws.map(k => (
                  <div key={k.word} className={`flex items-center justify-between p-2.5 rounded-xl border bg-white ${k.active ? 'border-emerald-100' : 'opacity-60 grayscale'}`}>
                    <span className={`text-[11px] font-bold ${k.active ? 'text-emerald-900' : 'text-slate-400 line-through'}`}>{k.word}</span>
                    <div className="flex items-center gap-1">
                      <button onClick={() => toggleKeyword(k.word, 'include')} className="p-1.5 text-slate-300">
                        {k.active ? <Eye size={14}/> : <EyeOff size={14}/>}
                      </button>
                      <button onClick={() => removeKeyword(k.word, 'include')} className="p-1.5 text-slate-300">
                        <X size={14}/>
                      </button>
                    </div>
                  </div>
            ...
            <section>
              ...
              <div className="space-y-2">
                {excludeKws.map(k => (
                  <div key={k.word} className={`flex items-center justify-between p-2.5 rounded-xl border bg-white ${k.active ? 'border-rose-100' : 'opacity-60 grayscale'}`}>
                    <span className={`text-[11px] font-bold ${k.active ? 'text-rose-900' : 'text-slate-400 line-through'}`}>{k.word}</span>
                    <div className="flex items-center gap-1">
                      <button onClick={() => toggleKeyword(k.word, 'exclude')} className="p-1.5 text-slate-300">
                        {k.active ? <Eye size={14}/> : <EyeOff size={14}/>}
                      </button>
                      <button onClick={() => removeKeyword(k.word, 'exclude')} className="p-1.5 text-slate-300">
                        <X size={14}/>
                      </button>
                    </div>
                  </div>
          ...
          <div className="p-4 bg-white border-t lg:hidden">
            <button onClick={handleSaveAll} className="w-full py-3 bg-indigo-600 text-white rounded-xl font-black text-xs uppercase tracking-widest flex items-center justify-center gap-2">
