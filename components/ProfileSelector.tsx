...
import { Plus, Users, ShieldCheck, ArrowRight, LogOut, RefreshCw, FolderPlus, FileX, Microscope, Trash2, Mail, Globe, CheckCircle2, Activity } from 'lucide-react';
    ...
    <div className="min-h-screen bg-[#F8FAFC] flex flex-col">
       <header className="bg-white border-b border-slate-100 px-6 md:px-8 py-4 flex items-center justify-between shadow-sm sticky top-0 z-50">
          <div className="flex items-center gap-3">
             <div className="h-10 w-10 bg-indigo-700 rounded-xl flex items-center justify-center text-white font-black text-lg shadow-lg shadow-indigo-100">
                <Microscope size={20} />
             </div>
             <div className="flex flex-col">
                <span className="font-black text-slate-900 tracking-tighter text-xl leading-none uppercase">Screeny</span>
                <span className="text-[8px] font-black text-slate-400 uppercase tracking-[0.2em]">by SRSG</span>
             </div>
          </div>
          <div className="flex items-center gap-4">
             <div className="hidden sm:flex flex-col items-end mr-2">
                <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Researcher</span>
       ...
       <main className="max-w-6xl mx-auto w-full p-4 md:p-10 flex-1">
          {step === 'shared-confirm' ? (
            <div className="max-w-xl mx-auto bg-white rounded-[40px] shadow-2xl p-12 text-center animate-in zoom-in duration-300 border border-slate-100">
               ...
               <h2 className="text-3xl font-black text-slate-900 mb-4">Sync Successful!</h2>
               <p className="text-slate-500 font-medium mb-8 leading-relaxed">
                  The project has been saved to the Cloud. <br/>
                  Once <span className="text-indigo-600 font-bold">{lastSharedWith}</span> logs in, they will see this project in their dashboard.
               </p>
            ...
            <div className="bg-white rounded-[40px] shadow-2xl p-8 md:p-12 border border-slate-200 animate-in fade-in zoom-in duration-300">
               <h2 className="text-3xl font-black text-slate-900 mb-8">New Systematic Review</h2>
               <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-6">
                     <div>
                        ...
                        <input value={name} onChange={e => setName(e.target.value)} placeholder="e.g., Diabetes 2024 Review" className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-4 focus:ring-indigo-500/10 outline-none font-bold text-slate-900" />
                     </div>
                     ...
                     <div>
                        <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 flex items-center gap-2"><Mail size={12}/> Collaborator Email</label>
                        <input value={emails} onChange={e => setEmails(e.target.value)} placeholder="colleague@research.edu" className="w-full px-5 py-3 bg-white border border-slate-200 rounded-xl font-bold" />
                        <p className="text-[9px] text-slate-400 mt-2 italic">* Use the email your colleague registered on Screeny.</p>
                     </div>
                  </div>
                  <div className="space-y-6">
                     <div className="p-6 bg-emerald-50 rounded-3xl border border-emerald-100">
                        <label className="block text-[10px] font-black text-emerald-600 uppercase mb-2">Inclusion Keywords</label>
                        <textarea value={incKey} onChange={e => setIncKey(e.target.value)} placeholder="human, clinical trial..." className="w-full p-4 bg-white border border-emerald-200 rounded-2xl text-sm font-bold h-24 outline-none focus:border-emerald-500" />
                     </div>
                     <div className="p-6 bg-rose-50 rounded-3xl border border-rose-100">
                        <label className="block text-[10px] font-black text-rose-600 uppercase mb-2">Exclusion Keywords</label>
                        <textarea value={excKey} onChange={e => setExcKey(e.target.value)} placeholder="animal, meta-analysis..." className="w-full p-4 bg-white border border-rose-200 rounded-2xl text-sm font-bold h-24 outline-none focus:border-rose-500" />
                     </div>
                  </div>
               </div>
               <div className="flex gap-4 mt-10">
                  ...
                  <button onClick={handleStartCreate} className="flex-1 py-4 bg-indigo-600 text-white rounded-2xl font-black shadow-xl shadow-indigo-100 hover:bg-indigo-700 transition-all flex items-center justify-center gap-2">Create & Share <ArrowRight size={20}/></button>
               </div>
            </div>
          ) : (
            <>
               <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12">
                  <div>
                     <h1 className="text-4xl font-black text-slate-900 tracking-tighter">My Research</h1>
                     <div className="flex items-center gap-4 mt-1">
                        <p className={`font-bold uppercase text-[10px] tracking-[0.2em] flex items-center gap-2 ${isRefreshing ? 'text-indigo-600' : 'text-slate-400'}`}>
                           <Activity size={10} className={isRefreshing ? 'animate-pulse' : ''} /> {isRefreshing ? 'Cloud Syncing...' : 'Database Connected'}
                        </p>
                        <div className="h-4 w-px bg-slate-200"></div>
                        <p className="text-emerald-500 font-bold uppercase text-[10px] tracking-[0.2em] flex items-center gap-2">
                           <Globe size={10} /> Collaboration Active
                        </p>
                     </div>
                  </div>
               ...
               <div className="space-y-16">
                  <section>
                    <h2 className="text-xs font-black text-slate-400 uppercase tracking-[0.25em] mb-6 flex items-center gap-2">
                        <FolderPlus size={14}/> Owned Projects ({myProjects.length})
                    </h2>
                    {myProjects.length === 0 ? (
                        <div className="bg-white rounded-[40px] border border-slate-200 border-dashed p-12 text-center">
                            <p className="text-slate-400 font-bold text-sm">No projects yet. Start by creating a New Project.</p>
                        </div>
                    ) : (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {myProjects.map(p => (
                               <div key={p.name} className="relative group h-full">
                                  <div onClick={() => onSelect(p.name)} className="w-full bg-white p-8 rounded-[35px] border border-slate-100 shadow-sm flex flex-col text-left group-hover:border-indigo-500 group-hover:shadow-2xl group-hover:shadow-indigo-500/5 transition-all relative overflow-hidden h-full cursor-pointer">
                                     <div className="flex items-center gap-4 mb-8">
                                        <div className="h-12 w-12 bg-indigo-600 rounded-2xl flex items-center justify-center text-white font-black text-lg">{p.name[0].toUpperCase()}</div>
                                        <div className="pr-8">
                                           <h3 className="font-black text-slate-900 text-lg leading-tight group-hover:text-indigo-600 transition-colors">{p.name}</h3>
                                           <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">Created: {p.createdAt}</p>
                                        </div>
                                     </div>
                                     <div className="space-y-2 mt-auto">
                                        <div className="flex items-center justify-between px-4 py-2 bg-slate-50 rounded-xl border border-slate-100">
                                           <span className="text-[10px] font-black text-slate-400 uppercase">Articles</span>
                                           <span className="text-xs font-black text-slate-900">{p.session?.articles?.length || 0}</span>
                                        </div>
                                     </div>
                                     <div className="mt-6 flex items-center gap-2 text-indigo-600 font-black text-[10px] uppercase opacity-0 group-hover:opacity-100 transition-opacity">Enter Room <ArrowRight size={14}/></div>
                                  </div>
                                  <div className="absolute top-8 right-8 z-[60]">
                                      <button 
                                        onClick={(e) => { e.stopPropagation(); e.preventDefault(); onDelete(p.name); }}
                  ...
                  <section>
                    <h2 className="text-xs font-black text-indigo-500 uppercase tracking-[0.25em] mb-6 flex items-center gap-2">
                        <Users size={14}/> Shared with Me ({sharedWithMe.length})
                    </h2>
                    {sharedWithMe.length === 0 ? (
                        <div className="bg-indigo-50/20 rounded-[40px] border border-indigo-100 border-dashed p-12 text-center">
                            <Users size={32} className="text-indigo-200 mb-4 mx-auto" />
                            <p className="text-indigo-400 font-bold text-sm">No shared projects found.</p>
                        </div>
                    ) : (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {sharedWithMe.map(p => (
                               <div key={p.name} onClick={() => onSelect(p.name)} className="w-full bg-white p-8 rounded-[35px] border border-indigo-200 bg-indigo-50/5 shadow-sm flex flex-col text-left group hover:border-indigo-500 transition-all cursor-pointer h-full">
                                  <div className="flex items-center gap-4 mb-8">
                                     <div className="h-12 w-12 bg-indigo-400 rounded-2xl flex items-center justify-center text-white font-black text-lg">{p.name[0].toUpperCase()}</div>
                                     <div>
                                        <h3 className="font-black text-slate-900 text-lg leading-tight flex items-center gap-2">{p.name} <Globe size={14} className="text-indigo-400"/></h3>
                                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">Owner: {p.ownerEmail?.split('@')[0]}</p>
                                     </div>
                                  </div>
